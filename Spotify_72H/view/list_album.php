...
<script>
    ...
    const url = `http://localhost:8000/albums/?page=${page}&limit=${limit}`;
    ...
    fetch(url).then(
        ...
        .then(data => {
            ...
            data.forEach(e => {
                $d = e.name;
                document.getElementById("a").innerHTML += `<li class='list-group-item'><a href="album.php?id=${e.id}">` + $d + "</a></li>";
                console.log(e.name);
            })
    ...
    function previous() {
        ...
        previous.onclick = () => {
            window.location.href = "list_album.php?page=" + <?= $_GET['page'] - 1 ?> + "&limit=10";
        }
    }
